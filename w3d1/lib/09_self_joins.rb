...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    SELECT
      ...
      finish.company
    FROM
      routes start JOIN stops transfer
      ON (start.stop_id = transfer.id)
      JOIN routes finish
      ON (finish.stop_id = transfer.id)
    WHERE
      (start.num, start.company) IN (
          SELECT
            start.num, start.company
          FROM
            routes start JOIN stops first_stop
            ON (first_stop.id = start.stop_id)
          WHERE
            first_stop.name = 'Craiglockhart'
        )
      AND
      (finish.num, finish.company) IN (
        SELECT
          finish.num, finish.company
        FROM
          routes finish JOIN stops last_stop
          ON (last_stop.id = finish.stop_id)
        WHERE
          last_stop.name = 'Sighthill'
        )
